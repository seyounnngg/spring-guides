# Spring Boot Getting Started Guides 학습 기록

Spring 공식 Getting Started Guides를 단계별로 따라 구현하고,
직접 다시 구현하고 기능을 확장하며 학습한 기록입니다.

## 진행 현황

| 단계                                  | Guide                                            | 상태   |
|-------------------------------------|--------------------------------------------------|------|
| [01](./01-rest-service)             | Building a RESTful Web Service                   | ✅ 완료 |
| [02](./02-serving-web-content)      | Serving Web Content with Spring MVC              | ✅ 완료 |
| [03](./03-handling-form-submission) | Handling Form Submission                         | ✅ 완료 |
| [04](./04-validating-form-input)    | Validating Form Input                            | ✅ 완료 |
| [05](./05-uploading-files)          | Uploading Files                                  | ✅ 완료 |
| [06](./06-accessing-data-jpa)       | Accessing Data with JPA                          | ✅ 완료 |
| [07](./07-accessing-data-rest)      | Accessing JPA Data with REST                     | ✅ 완료 |
| 08                                  | Building a Hypermedia-Driven RESTful Web Service | ⬜    |
| 09                                  | Testing the Web Layer                            | ⬜    |
| 10                                  | Securing a Web Application                       | ⬜    |

## 학습 방식 (각 Guide 공통)

1. Guide를 보고 따라 구현
2. README에 배운 내용 정리 (개념, 핵심 Annotation, 실행 화면, 느낀 점)
3. Guide 없이 다시 구현
4. 기능 확장 (CRUD, 검색, Validation 등 중 1개 이상)
5. Git 커밋 & PR