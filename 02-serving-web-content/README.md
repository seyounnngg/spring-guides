...
가이드 페이지와 기존 코드를 보지 않고 GreetingController, greeting.html을 처음부터 다시 작성해봄.

- Model이 "내가 직접 만드는 객체"가 아니라 Spring이 요청 처리 시 자동으로 만들어서 파라미터에 넣어주는 객체라는 점을 다시 짚어봄 (제어의 역전 개념)
- @RequestParam 축약형(defaultValue만 쓰는 것)과 완전한 형태(name, required, defaultValue 다 명시하는 것)이 동일하게 동작함을 확인

## Step 4: 기능 확장 — 인사말 목록 화면 추가

기존 /greeting은 한 번에 하나의 인사말만 화면에 보여줬는데,
지금까지 요청된 인사말을 서버 메모리에 저장해두고
/greetings 경로로 접속하면 전체 목록을 화면에 렌더링하도록 확장했다.

### 추가한 것

- CopyOnWriteArrayList<String>으로 인사말 이력 저장
- GET /greetings 엔드포인트 추가
- greetings-list.html에서 th:each로 리스트 반복 렌더링
...
- th:each="변수명 : ${리스트이름}" 문법에서, 왼쪽(반복 변수명)은 자유 작명이고 오른쪽(${...})은 컨트롤러의 addAttribute 키와 정확히 일치해야 한다는 걸 이해함
- REST API(@RestController, JSON 응답)와 화면 렌더링(@Controller, Thymeleaf)의 역할 차이를 명확히 구분하게 됨